...
/**
 * Converte uma string de data (YYYY-MM-DD) e hora opcional (HH:MM) para objeto Date no horário de Brasília
 */
...
/**
 * Formata data de forma relativa (hoje, amanhã, etc) no horário de Brasília
 */
...
/**
 * Formata hora no padrão HH:MM (horário de Brasília)
 */
...
/**
 * Formata data completa com hora no padrão brasileiro (horário Brasília)
 */
...
async function carregarTarefas() {
  ...
  try {
    ...
    container.querySelectorAll('.btn-excluir-tarefa').forEach(btn => {
      btn.addEventListener('click', () => excluirTarefa(btn.dataset.id));
    });

    container.querySelectorAll('.btn-detalhes-tarefa').forEach(btn => {
      btn.addEventListener('click', () => {
        const tarefa = data.find(t => t.id === btn.dataset.id);
